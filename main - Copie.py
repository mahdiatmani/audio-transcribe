...
HUGGINGFACE_MODEL = os.getenv("HUGGINGFACE_MODEL", "openai/whisper-large-v3")
...
def transcribe_with_huggingface_free(audio_bytes: bytes) -> str:
    ...
    if not HUGGINGFACE_API_KEY:
        return "⚠️ Hugging Face API key not configured. Add it to your .env file!"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return result.get("text", "Transcription completed successfully.")
...
async def transcribe_audio(
    ...
    if user["usage_minutes"] + estimated_duration > tier_limit:
        raise HTTPException(
            status_code=403,
            detail=f"Usage limit reached. Please upgrade."
        )
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("🚀 TranscribeAI Backend - 100% FREE")
    print("=" * 50)
    print(f"📝 API Docs: http://localhost:{os.getenv('PORT', 8000)}/docs")
    print(f"🔑 HuggingFace Key: {'✅ Configured' if HUGGINGFACE_API_KEY else '❌ Not configured'}")
    print("💰 Cost: $0.00")
    print("=" * 50)
